...
export function dedupeAndExtract(records = [], uniqueField1, uniqueField2, uniqueField3, uniqueField4, uniqueField5, propToConcat, arrayOfConcatProps, arrayOfDuplicateObjects) {
  return records.reduce((accumulator, current) => {
    ...
    const duplicateObject = accumulator.find(item =>
     item.disposition.get(uniqueField1).toString() === current.disposition.get(uniqueField1).toString()
     && item.disposition.get(uniqueField2).toString() === current.disposition.get(uniqueField2).toString()
     && item.disposition.get(uniqueField3).toString() === current.disposition.get(uniqueField3).toString()
     && item.disposition.get(uniqueField4).toString() === current.disposition.get(uniqueField4).toString()
     && item.disposition.get(uniqueField5).toString() === current.disposition.get(uniqueField5).toString(),
    );
