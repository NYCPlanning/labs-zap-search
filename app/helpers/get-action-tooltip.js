...
export function getActionTooltip([actioncode]) {
  switch(actioncode) {
    ...
    case 'HD':
      return '(ULURP) Disposition of Urban Renewal Site';
    case 'HF':
      return 'Community Dev. Application/Amendment';
    case 'HG':
      return '(Non-ULURP) Urban Renewal Designation';
    case 'HI':
      return '(Non-ULURP) Landmarks -Individual Sites ';
    case 'HK':
      return '(Non-ULURP) Proposed New York City Landmarks and Historic Districts are submitted to the Department of City Planning by the Landmarks Preservation Commission to ensure that the landmark designation does not conflict with the Zoning Resolution, projected public improvements or any plans for development, growth, improvement, or renewal in the vicinity of the landmark. &nbsp;Landmark designations are non-ULURP actions.';
    case 'HL':
      return 'Housing/Urban Renewal/Pub Ben Corp Lease';
    case 'HM':
      return 'Currently Residential/Not Low-Income';
    case 'HN':
      return '(Non-ULURP) Urban Development Action Area -UDAAP Non-ULURP ';
    case 'HO':
      return '(ULURP) Housing Application (Plan and Project)';
    case 'HP':
      return '(ULURP) Plan & Project/Land Disposition Agreement (LDA) ';
    case 'HR':
      return 'Assignments & Transfers';
    case 'HS':
      return 'Special District/Mall Plan/REMIC NPA';
    case 'HU':
      return '(ULURP) Urban Renewal Plan and Amendments ';
    case 'HZ':
      return 'Preliminary Site Approval Application';
    case 'LD':
      return '(ULURP) Legal Document (NOC, NOR, RD) ';
    case 'MA':
    ...
    case 'MD':
      return '(Non-ULURP) Amended drainage plans are submitted to the Department of City Planning for a review of their consistency with the City Map. Drainage plans are non-ULURP actions submitted to DCP by the NYC Department of Environmental Protection.';
    case 'ME':
      return '(Non-ULURP) An easement allows for an entity or individual to have limited use of another’s property for a specific purpose. Please contact the Department’s Technical Review Division to discuss new easements or modifications to existing easements and the associated application requirements.';
    case 'MF':
      return 'Franchise Applic -Not Sidewalk Café';
    case 'ML':
      return '(ULURP) Landfill applications are application for the establishment of sanitary or waterfront landfills.';
    case 'MM':
    ...
    case 'PP':
      return '(ULURP) Dispositions of non-residential city-owned property are used to release property acquired by the city for use or development by another party.  This includes the sale, lease, or exchange of real property. This action is subject to the Uniform Land Use Review Procedure.';
    case 'PQ':
    ...
    case 'PX':
      return '(Non-ULURP) Applications for the leasing of office space by city agencies are typically filed by the Department of Citywide Administrative Services. Office space lease actions are non-ULURP and are required to move through the approval process on a specific timeline.';
    case 'RA':
      return '(Non-ULURP) A Zoning Authorization within the Special South Richmond Development District (SSRDD) is a discretionary action taken by the City Planning Commission (CPC) that modifies specific zoning requirements if certain findings have been met. Authorizations are not subject to ULURP review and the CPC does not hold public hearings on authorizations. The CPC generally refers such applications to the appropriate community board(s) for comments.';
    case 'RC':
      return '(Non-ULURP) A Special South Richmond Development District certification is a non-discretionary action taken by the City Planning Commission, or its Chairperson, informing the Department of Buildings that an as-of-right development has complied with specific conditions set forth in accordance with provisions of the Zoning Resolution.';
    case 'RS':
    ...
    case 'SC':
      return '(Non-ULURP) Special Natural Area Certifications ';
    case 'Study':
      return '(ULURP) Study';
    case 'TC':
    ...
    case 'VT':
      return 'Cable TV';
    case 'WR':
      return '(ULURP) Waterfront Revitalization Program ';
    case 'ZA':
      return '(Non-ULURP) An authorization is a discretionary action taken by the City Planning Commission (CPC) that modifies specific zoning requirements if certain findings have been met. Authorizations are not subject to ULURP review, and the CPC does not hold public hearings on authorizations. The CPC generally refers such applications to the appropriate community board(s) for comment.';
    case 'ZC':
      return '(Non-ULURP) Each Zoning Certification is a distinct land use action and requires specific land use application components.';
    case 'ZD':
    ...
    case 'ZM':
      return '(ULURP) A Zoning Map Amendment is a change in designation or a change in district boundaries for any zoning district on the New York City Zoning Map.  Zoning Map Amendments are discretionary actions subject to the Uniform Land Use Review Procedure.';
    case 'ZP':
      return 'Parking Special Permit/Incl non-ULURP Ext';
    case 'ZR':
      return '(non-ULURP) Zoning Text Amendments are changes to the text of the New York City Zoning Resolution.  Text Amendments do not require the Uniform Land Use Review Procedure, however, in practice; the Department of City Planning refers Zoning Text Amendments to the Community Board for review.';
    case 'ZS':
      return '(ULURP) A special permit is a discretionary action by the City Planning Commission, subject to ULURP review, or the Board of Standards and Appeals, which may modify use, bulk, or parking regulations if certain conditions and findings specific in the Zoning Resolution are met.';
    case 'ZX':
