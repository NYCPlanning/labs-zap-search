...
describe('Assignment Get', () => {
  ...
  test('Get correct assignment keys', async () => {
    ...
    const crmAssignmentsIsh = fs.readFileSync(`${rootPath}/test/test-data/crm-projects-with-lup-teams.json`).toString();

    scope
      .get(uri => uri.includes('api/data/v9.1/dcp_projects'))
      .reply(200, { value: JSON.parse(crmAssignmentsIsh), '@odata.context': '' });
