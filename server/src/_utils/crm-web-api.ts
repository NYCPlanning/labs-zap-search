...
export const CRMWebAPI = {
        ...
        if (!error && response.statusCode === 200) {
          ...
          const parseError = jsonText => {
            ...
            if (
              ...
              this.get(query, maxPageSize, options).then(resolve, reject);
            } else {
              reject(err);
            }
          };
  ...
  findDocumentLocation: function(entityID, folderName) {
    ...
    return this.get(
      `sharepointdocumentlocations?fetchXml=${fetchDocumentLocationXML}`
    ).then((response: any) => {
  ...
  getParentSiteLocation: async function() {
    ...
    return this.get(
      `sharepointsites?fetchXml=${fetchParentSiteLocationIdXML}`
    ).then((response: any) => {
  ...
  createDocumentLocation: async function(
    locationName,
    absURL,
    folderName,
    sharepointSiteID,
  ...
  uploadDocument: async function(
    ...
    if (!docLocation) {
      ...
      if (!parentSiteLocation)
        throw new Error("Sharepoint Site Location not found");
      const sharepointSiteID = parentSiteLocation["sharepointsiteid"];
      const entityDocLocation = await this.findEntityDocumentLocation(
        entityName,
        sharepointSiteID
      );
      if (!entityDocLocation)
        throw new Error("Entity Document Location not found");
      const entityDocName = entityDocLocation.name;
      const absoluteURL = `${
        parentSiteLocation["absoluteurl"]
  ...
  executeAction: async function(
    ...
    if (!entitySetName) query = actionName;
      ...
      query =
        entitySetName + "(" + guid + ")/Microsoft.Dynamics.CRM." + actionName;
    return this.create(query, data, headers);
  },
