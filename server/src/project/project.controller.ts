...
export class ProjectController {
  ...
  /**
   * Update project geometries from current project bbls.
   * Queries CRM for projectbbls, gets the union-ed geometries
   * from CARTO, and updates the geometry assocaiated with the
   * @param id
   */
  @Get("/projects/sync/:id")
  async syncProjectGeoms(@Param("id") id: string) {
    try {
      await this.projectService.syncProject(id);
      return {
        success: true
      };
    } catch (e) {
  ...
  @Get("/projects/")
  async index(
    @Query() query: ClientProjectQuery,
    @Query("skipTokenParams") skipTokenParams
  ) {
    try {
      if (skipTokenParams) {
