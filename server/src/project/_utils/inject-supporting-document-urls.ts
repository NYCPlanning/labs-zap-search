...
const S3_BUCKET_HOST =
  "https://labs-zap-supporting-documents.sfo2.digitaloceanspaces.com";
...
export async function injectSupportDocumentURLs(project) {
  ...
  const ulurpNumbers = project.actions
    ...
    .map(({ dcp_ulurpnumber }) =>
      dcp_ulurpnumber.match(/[A-Z]?([A-Z0-9]{6,7})[A-Z]{3}/)
    )
  ...
  try {
    ...
    const searchResults = await Promise.all([
      ...ulurpNumbers.map(ulurlp =>
        fetch(`${S3_BUCKET_HOST}/?prefix=comments/${ulurlp}`)
          .then(blob => blob.text())
          .then(text =>
            parseStringAsync(text).catch(error => console.log(error))
          )
      ),
      ...ulurpNumbers.map(ulurlp =>
        fetch(`${S3_BUCKET_HOST}/?prefix=letters-dob-hpd/${ulurlp}`)
          .then(blob => blob.text())
          .then(text =>
            parseStringAsync(text).catch(error => console.log(error))
          )
