...
export const rollupActionTypes = row => {
  row.actiontypes = row.actiontypes
    ? row.actiontypes
        .split(";")
        .map(at => actionTypesLookup[at])
        .join(";")
    : "";
};
...
export const transformActions = actions => {
  return actions
    .filter(
      ({
        "_dcp_action_value@OData.Community.Display.V1.FormattedValue": _dcp_action_value
      }) => Object.keys(actionTypesLookup).includes(_dcp_action_value)
    )
    .map(action => ({
      ...action,
