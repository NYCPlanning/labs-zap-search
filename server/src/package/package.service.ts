...
export class PackageService {
  ...
  async getPackageSharepointDocuments(packageDocumentLocation) {
    ...
    const { relativeurl, _regardingobjectid_value } = packageDocumentLocation;
    ...
    if (relativeurl) {
      try {
        ...
        if (e instanceof HttpException) {
          ...
          const errorMessage = `An error occured while constructing and looking up folder for package. Perhaps the package name or id is wrong.`;
    ...
    console.log(
      `Warning: Tried to load documents for a package but the Document Location "relativeurl" was null. Regarding object is ${_regardingobjectid_value}`
    );
  ...
  public async packageWithDocuments(projectPackage: any) {
    ...
    if (documentLocations === null) {
      console.log(
        `Warning: Tried to load documents for the Package "${dcp_name}", but no dcp_package_SharePointDocumentLocations was null.`
      );
    }
