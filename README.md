...
The frontend app can be served on your machine standalone by stubbing the backend using Mirage. The app has been configured with a few [Mirage Scenarios](https://github.com/NYCPlanning/labs-zap-search/tree/develop/client/mirage/scenarios), which represent different sets of data for different user types. You can select the scenario when serving the app:
...
#### Logging in (FLAG: Is this updated?) 

Sign in by going to `localhost:4200/login#access_token=test`

### Connecting to a local backend (FLAG: Is this updated?) 
...
### Connecting to staging or production (FLAG: Is this updated?) 
...
### Deploying (FLAG: Is this updated?) 
...
The API is a [Nest](https://github.com/nestjs/nest) api that reads from and writes to the CRM database.

(FLAG: Is this updated?)
```bash
# development
$ yarn run start
...
## Local development (FLAG: UPDATE THIS)
